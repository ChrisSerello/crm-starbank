...
const INIT = {
  ...
  filters:{search:"",product:"",operator:"",stage:""},
  rules: savedRules,
};
...
function LeadsTable({leads,dispatch,filters}){
  const flt=useMemo(()=>leads.filter(l=>{
  ...
  return(
    <div style={{padding:"28px 32px"}}>
      <div className="fu" style={{display:"flex",justifyContent:"space-between",alignItems:"flex-end",marginBottom:20}}>
        <div><div className="section-title">Leads</div><div className="section-sub">{flt.length} de {leads.length} registros</div></div>
          ...
          <table style={{width:"100%",borderCollapse:"collapse",fontSize:13}}>
            <thead>
              <tr style={{background:"rgba(90,70,50,.04)",borderBottom:"1px solid var(--border)"}}>
                <TH c="Nome / CPF"/><TH c="Órgão"/><TH c="Produtos"/><TH c="Estágio"/><TH c="Operador"/><TH c="Último contato"/><TH c="Documento"/><TH c=""/>
              </tr>
            </thead>
            <tbody>
              {flt.map(l=>{
              ...
              {flt.length===0&&<tr><td colSpan={8} style={{padding:"40px 0",textAlign:"center",color:"var(--text-muted)",fontSize:13}}>Nenhum lead encontrado</td></tr>}
            </tbody>
          </table>
        </div>
      </div>
