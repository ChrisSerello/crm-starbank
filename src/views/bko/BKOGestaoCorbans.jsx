...
function PortalRow({ label, value }) {
  ...
  return (
    <div style={{ display:'grid', gridTemplateColumns:'110px 1fr', gap:8, padding:'5px 0', borderBottom:'1px solid rgba(139,92,246,.1)' }}>
...
function PortalBadge({ style={} }) {
  return (
    <span style={{ display:'inline-flex', alignItems:'center', gap:4, fontSize:9, fontWeight:700, padding:'2px 7px', borderRadius:4, background:'rgba(139,92,246,.12)', color:'#7C3AED', border:'1px solid rgba(139,92,246,.22)', letterSpacing:'.04em', ...style }}>
      ⚡ Portal
    </span>
  );
}

// ── Seletor de produção mensal ────────────────────────────────────────────────
function ProducaoSelector({ value, onChange }) {
  return (
    <div>
      <label style={{ display:'block', fontSize:10, fontWeight:700, color:'var(--text-muted)', textTransform:'uppercase', letterSpacing:'.07em', marginBottom:8 }}>
        Produção mensal estimada
      </label>
      <div style={{ display:'flex', gap:7, flexWrap:'wrap' }}>
        {PRODUCAO_OPCOES.map(op => {
          const ativo = value === op;
          return (
            <button key={op} type="button" onClick={() => onChange(ativo ? '' : op)}
              style={{ padding:'7px 14px', borderRadius:8, fontSize:12, fontWeight:500, cursor:'pointer', transition:'all .15s', fontFamily:'var(--font)',
                background: ativo ? '#3B5BDB' : 'var(--bg-card)',
                color:      ativo ? '#fff'    : 'var(--text-secondary)',
                border:     ativo ? '1.5px solid #3B5BDB' : '1.5px solid var(--border)',
              }}
              onMouseEnter={e => { if (!ativo) { e.currentTarget.style.borderColor='#3B5BDB'; e.currentTarget.style.color='#3B5BDB'; }}}
              onMouseLeave={e => { if (!ativo) { e.currentTarget.style.borderColor='var(--border)'; e.currentTarget.style.color='var(--text-secondary)'; }}}>
              {op}
            </button>
          );
        })}
...
function GCCard({ c, onSelect, setDragId, onMove, profile }) {
  ...
  const initials = c.responsavel_nome ? c.responsavel_nome.split(' ').slice(0,2).map(n=>n[0]).join('').toUpperCase() : null;

  return (
    <>
      <div draggable onDragStart={() => setDragId(c.id)}
        onClick={(e) => { if (!e.defaultPrevented) onSelect(c); }}
        style={{ background:'var(--bg-card)', border:'1px solid var(--border)', borderLeft:`3px solid ${stage?.color||'#8B5CF6'}`, borderRadius:8, padding:'10px 12px', marginBottom:7, cursor:'pointer', position:'relative', transition:'border-color .15s, box-shadow .15s' }}
        onMouseEnter={e => { e.currentTarget.style.boxShadow=`0 2px 8px ${stage?.color}18`; e.currentTarget.style.borderColor=stage?.color||'#8B5CF6'; }}
        onMouseLeave={e => { e.currentTarget.style.boxShadow='none'; e.currentTarget.style.borderColor='var(--border)'; e.currentTarget.style.borderLeftColor=stage?.color||'#8B5CF6'; }}>

        {c.origem_portal && (
          <span style={{ display:'inline-flex', alignItems:'center', gap:3, fontSize:9, fontWeight:700, padding:'1px 6px', borderRadius:3, background:'rgba(139,92,246,.1)', color:'#7C3AED', marginBottom:6, letterSpacing:'.03em' }}>⚡ Portal</span>
        )}

        <div style={{ fontSize:12, fontWeight:600, color:'var(--text-primary)', lineHeight:1.3, marginBottom:7, paddingRight:22 }}>{c.nome}</div>

        {cnpj && <div style={{ display:'flex', alignItems:'center', gap:5, fontSize:11, color:'var(--text-muted)', marginBottom:3 }}><span style={{ fontSize:12, opacity:.6 }}>🏢</span> {cnpj}</div>}
        {tel  && <div style={{ display:'flex', alignItems:'center', gap:5, fontSize:11, color:'var(--text-muted)', marginBottom:3 }}><span style={{ fontSize:12, opacity:.6 }}>📞</span> {tel}</div>}
        {c.producao_mensal && <div style={{ display:'flex', alignItems:'center', gap:5, fontSize:11, color:'var(--text-muted)', marginBottom:3 }}><span style={{ fontSize:12 }}>📊</span> {c.producao_mensal}</div>}
        {c.saldo_produzido && <div style={{ display:'flex', alignItems:'center', gap:5, fontSize:11, fontWeight:700, color:'#10B981', marginBottom:3 }}><span style={{ fontSize:12 }}>💰</span> {c.saldo_produzido}</div>}

        <div style={{ display:'flex', alignItems:'center', gap:6, marginTop:8, paddingTop:7, borderTop:'1px solid var(--border)' }}>
          <div style={{ width:18, height:18, borderRadius:'50%', flexShrink:0, background:initials?'rgba(59,91,219,.12)':'rgba(0,0,0,.06)', display:'flex', alignItems:'center', justifyContent:'center', fontSize:8, fontWeight:700, color:initials?'#3B5BDB':'var(--text-faint)' }}>{initials || '—'}</div>
          <span style={{ fontSize:10, color:initials?'var(--text-muted)':'var(--text-faint)' }}>{c.responsavel_nome || 'Sem responsável'}</span>
        </div>

        <button ref={btnRef} onClick={openMenu}
          style={{ position:'absolute', top:8, right:8, background:'rgba(0,0,0,.06)', border:'none', borderRadius:5, cursor:'pointer', width:20, height:20, display:'flex', alignItems:'center', justifyContent:'center', fontSize:11, color:'var(--text-muted)', lineHeight:1, padding:0 }}>⋯</button>
      </div>
      ...
      {menuOpen && ReactDOM.createPortal(
        <div data-gc-menu="1" onMouseDown={e => e.stopPropagation()}
          ...
          {GC_STAGES.filter(s => s.id !== c.estagio).map(s => (
            <button key={s.id} onMouseDown={e => { e.stopPropagation(); onMove(c.id, s.id); setMenuOpen(false); }}
...
function GCCol({ s, corbans, dragId, setDragId, onMove, onSelect, profile, collapsed, onToggleCollapse }) {
  ...
  return (
    <div style={{ minWidth:collapsed?44:180, width:collapsed?44:210, flexShrink:0, background:over?`${s.color}06`:'rgba(0,0,0,.02)', border:`1px solid ${over?s.color+'40':'var(--border)'}`, borderRadius:12, padding:'10px 8px', transition:'all .22s cubic-bezier(.4,0,.2,1)', display:'flex', flexDirection:'column' }}
      ...
      {collapsed ? (
        ...
        <>
          <div style={{ display:'flex', alignItems:'center', gap:6, marginBottom:10, padding:'0 3px', flexShrink:0 }}>
            ...
            <button onClick={onToggleCollapse} title="Recolher" style={{ background:'none', border:'none', cursor:'pointer', color:'var(--text-faint)', fontSize:12, padding:'0 2px', lineHeight:1 }}
          ...
          <div style={{ flex:1, overflowY:'auto', minHeight:40, paddingRight:2, scrollbarWidth:'thin', scrollbarColor:'rgba(0,0,0,.1) transparent' }}>
            {items.map(c => <GCCard key={c.id} c={c} onSelect={onSelect} setDragId={setDragId} onMove={onMove} profile={profile}/>)}
...
function GCDetalhe({ corban, onClose, onSave, onDelete, responsaveis }) {
  ...
  const save = async () => {
    ...
    const { error } = await supabase.from('gestao_corbans').update({
      nome:            updated.nome,
      ...
      pj_cnpj:         updated.pj_cnpj,
      pj_telefone:     updated.pj_telefone,
      producao_mensal: updated.producao_mensal,
  ...
  return (
    <div style={{ position:'fixed', inset:0, background:'rgba(0,0,0,.45)', zIndex:1000, display:'flex', alignItems:'center', justifyContent:'center' }}
      ...
      <div style={{ background:'var(--bg-card)', borderRadius:16, padding:28, width:520, maxHeight:'90vh', overflowY:'auto', boxShadow:'0 20px 60px rgba(0,0,0,.3)' }}>
        ...
        <div style={{ display:'flex', justifyContent:'space-between', alignItems:'flex-start', marginBottom:20 }}>
          <div>
            ...
            <div style={{ marginTop:6 }}>
              <span style={{ fontSize:10, padding:'2px 8px', borderRadius:99, fontWeight:700, background:`${stage?.color}18`, color:stage?.color }}>{stage?.label}</span>
            </div>
          </div>
        ...
        {/* Dados do portal */}
        {form.origem_portal && (
          <div style={{ border:'1px solid rgba(139,92,246,.22)', borderRadius:12, marginBottom:18, overflow:'hidden', background:'rgba(139,92,246,.03)' }}>
            <button onClick={() => setShowPortal(v => !v)}
              style={{ width:'100%', display:'flex', justifyContent:'space-between', alignItems:'center', padding:'10px 14px', background:'none', border:'none', cursor:'pointer', fontFamily:'var(--font)' }}>
              <span style={{ fontSize:11, fontWeight:700, color:'#7C3AED', display:'flex', alignItems:'center', gap:6 }}>
                ⚡ Dados preenchidos no portal
                <span style={{ fontSize:9, fontWeight:600, padding:'1px 6px', borderRadius:4, background:'rgba(139,92,246,.12)', color:'#7C3AED' }}>
                  {form.portal_status === 'rascunho' ? 'incompleto' : 'completo'}
                </span>
              </span>
              <span style={{ fontSize:14, color:'#7C3AED', transform:showPortal?'rotate(180deg)':'none', transition:'transform .2s', display:'inline-block' }}>▾</span>
            </button>
            {showPortal && (
              <div style={{ padding:'0 14px 14px' }}>
                <div style={{ fontSize:10, fontWeight:700, color:'rgba(139,92,246,.7)', textTransform:'uppercase', letterSpacing:'.07em', padding:'8px 0 6px' }}>Empresa</div>
                ...
                {hasPerfilComercial && (
                  <>
                    <div style={{ fontSize:10, fontWeight:700, color:'rgba(139,92,246,.7)', textTransform:'uppercase', letterSpacing:'.07em', padding:'12px 0 6px' }}>Perfil Comercial</div>
                ...
                {hasRepresentante && (
                  <>
                    <div style={{ fontSize:10, fontWeight:700, color:'rgba(139,92,246,.7)', textTransform:'uppercase', letterSpacing:'.07em', padding:'12px 0 6px' }}>Representante Legal</div>
                ...
                {hasBanco && (
                  <>
                    <div style={{ fontSize:10, fontWeight:700, color:'rgba(139,92,246,.7)', textTransform:'uppercase', letterSpacing:'.07em', padding:'12px 0 6px' }}>Dados Bancários</div>
        ...
        <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:12, marginBottom:12 }}>
          <div style={{ gridColumn:'1/-1' }}>
            <label style={{ display:'block', fontSize:10, fontWeight:700, color:'var(--text-muted)', textTransform:'uppercase', letterSpacing:'.07em', marginBottom:5 }}>Nome Fantasia *</label>
            <input className="inp" value={form.nome||''} onChange={e => set('nome', e.target.value)} placeholder="Nome fantasia do corban"/>
          </div>
          <div>
            <label style={{ display:'block', fontSize:10, fontWeight:700, color:'var(--text-muted)', textTransform:'uppercase', letterSpacing:'.07em', marginBottom:5 }}>CNPJ</label>
            <input className="inp" value={maskCNPJ(form.pj_cnpj||'')} onChange={e => set('pj_cnpj', e.target.value.replace(/\D/g,''))} placeholder="00.000.000/0000-00" maxLength={18}/>
          </div>
          <div>
            ...
            <input className="inp" value={maskTel(form.pj_telefone||'')} onChange={e => set('pj_telefone', e.target.value.replace(/\D/g,''))} placeholder="(00) 00000-0000" maxLength={15}/>
          </div>
          <div>
          ...
          <div style={{ gridColumn:'1/-1' }}>
            <ProducaoSelector value={form.producao_mensal||''} onChange={v => set('producao_mensal', v)}/>
          </div>
          <div style={{ gridColumn:'1/-1' }}>
...
function GCNovo({ onClose, onSave, profile, responsaveis }) {
  ...
  const save = async () => {
    if (!form.nome.trim()) { setMsg('Nome Fantasia é obrigatório.'); return; }
    setSaving(true);
    const resp = responsaveis.find(r => r.id === form.responsavel_id);
    const novo = {
      id:               crypto.randomUUID(),
      nome:             form.nome.trim(),
      pj_cnpj:          form.pj_cnpj || null,
      pj_telefone:      form.pj_telefone || null,
      producao_mensal:  form.producao_mensal || null,
    ...
    <div style={{ position:'fixed', inset:0, background:'rgba(0,0,0,.45)', zIndex:1000, display:'flex', alignItems:'center', justifyContent:'center' }}
      ...
      <div style={{ background:'var(--bg-card)', borderRadius:16, padding:28, width:480, maxHeight:'90vh', overflowY:'auto', boxShadow:'0 20px 60px rgba(0,0,0,.3)' }}>
        ...
        <div style={{ marginBottom:12 }}>
          <label style={{ display:'block', fontSize:10, fontWeight:700, color:'var(--text-muted)', textTransform:'uppercase', letterSpacing:'.07em', marginBottom:5 }}>Nome Fantasia *</label>
          <input className="inp" value={form.nome} onChange={e => set('nome', e.target.value)} placeholder="Nome fantasia do corban" autoFocus/>
        </div>
...
export function BKOGestaoCorbans({ profile }) {
  ...
  useEffect(() => {
    supabase.from('profiles').select('id,nome').eq('modulo','bko').eq('acesso_gestao_corban',true).order('nome')
      .then(({ data }) => setResponsaveis(data || []));
  }, []);

  useEffect(() => {
    supabase.from('gestao_corbans').select('*').order('created_at', { ascending:false })
    ...
    <div style={{ display:'flex', flexDirection:'column', height:'100%', overflow:'hidden' }}>
      ...
      {loading ? (
        ...
        <div ref={colsRef} style={{ display:'flex', gap:7, overflowX:'auto', overflowY:'hidden', padding:'0 20px 16px', flex:1, minHeight:0, alignItems:'stretch', scrollbarWidth:'thin', scrollbarColor:'rgba(59,91,219,.4) transparent' }}>
          {GC_STAGES.map(s => (
            <GCCol key={s.id} s={s} corbans={filtered} dragId={dragId} setDragId={setDragId}
              onMove={onMove} onSelect={setSelected} profile={profile}
              collapsed={collapsedCols.has(s.id)} onToggleCollapse={() => toggleCol(s.id)}/>
          ))}
        </div>
      )}

      {selected && <GCDetalhe corban={selected} onClose={() => setSelected(null)} onSave={onSaveDetalhe} onDelete={onDeleteDetalhe} responsaveis={responsaveis}/>}
      {novoOpen  && <GCNovo   onClose={() => setNovoOpen(false)} onSave={onSaveNovo} profile={profile} responsaveis={responsaveis}/>}
    </div>
  );
}